...
definitions remain out of scope. See `test_inherited_members.py`.

Phase 5c-2 adds REDEFINITION (no construct row moves). `part x :>> y` redefines an
existing feature (a `Redefinition`, the KerML Subsetting subkind -- one new kernel
class, 36 -> 37). The redefined feature resolves through owned/inherited/imported
scopes excluding the redefining feature itself, so a bare `:>> x` redefines the
inherited `x`; declaring a redefining feature shadows an inherited-name conflict, so
a conflict that is `ambiguous-name` when merely inherited is RESOLVED by redefining
one supertype's feature (`:>> A::x`). Validation adds `unresolved-redefinition`
...
Phase 5e adds FEATURE-CHAIN CONNECTOR ENDPOINTS (`connect a.b to c.d`, + succession
first/then, flow from/to) -- the explicit extension deferred by Phase 9's non-chain
endpoints (one new kernel class, `FeatureChaining`, 37 -> 38). A chain resolves
step-by-step -- head nearest-first, each `.step` as a member of the previous
feature's type -- and becomes a synthesized anonymous Feature owning ordered
FeatureChainings, set as the connector's source/target. It is created only after both
ends resolve (atomic; a broken step is `broken-connection-end`), gets no implicit
base, is never exported as a member, and renders/fingerprints as the dotted path.
Validation adds model-derived `broken-feature-chain`. Feature chains in non-endpoint
positions and FeatureChainExpression are out of scope. See `test_feature_chains.py`.
